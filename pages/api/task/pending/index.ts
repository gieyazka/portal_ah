...
import { filterStore, task } from '@/types/next-auth';
...
export default async function handler(
    ...
    if (req.method === 'POST') {
        try {
            ...
            let dataArr = []
            const { user, filterStore: filterStore } = req.body.data
            // const getWorkflowTask = await axios({
            //     method: 'POST',
            //     url: `${process.env.NEXT_PUBLIC_WORKFLOW_URL}/custom_api/find_user_approve`,
            //     data: req.body,
            //     validateStatus: () => true,
            // })
            // dataArr = getWorkflowTask.data
            //
            let queryCarBooking = `?status=free&managerApprove_null=true&managerEmail=${user.email}`
            let queryEGatePass = `?status=waiting&manager=${user.email}&doc_type=E-Gate-Pass`
