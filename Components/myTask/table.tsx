...
const RenderExportTable = ({ headerTable, loading, data }: { headerTable: headerTable[]; loading: boolean; data: {}[] | undefined }) => {
      ...
      <div className='mb-4 flex items-end justify-between '>
        ...
        >
          ...
          <input
            ...
            onKeyUp={(e) => filterStore.handleChangeFilterStr(e.target.value)}
            ...
            {headerTable.map((key, index) => {
              return (
                <div
                  ...
                  className={`py-2 text-center  whitespace-nowrap cursor-pointer   font-medium ${!key.width && "flex-1"} `}
                  key={index}
                  style={{
          ...
          <div className='flex  flex-col gap-1'>
            {(loading
              ...
              return (
                <Tooltip
                  key={i.toString()}
                  title={checkNeedFileLeave ? `Need file for type ${task?.data?.type?.labelTH}` : undefined}
                >
                  <div
