...
import { useActionDialogStore, useDialogStore, usePreviewStore, useViewStore } from "@/store/store";
...
const RenderDialog = (props: {
  ...
  const handleOpenSubDialog = (task: task, type: string | undefined, action: boolean, swrResponse: SWRResponse | undefined) => {
        ...
        {task !== undefined && (
          ...
          >
            ...
            <div className='flex justify-between   items-center py-2 px-4 bg-[#EEF1F8]'>
              ...
              >
                {task.data.flowName === "leave_flow" ? "E-Leave" : task.data.flowName}
              </Typography>
              <Typography
                component='p'
            ...
            {/* </DialogTitle> */}
            <DialogContent dividers={true}>{type === undefined && <CheckFlow task={task} />}</DialogContent>
            {_fn.checkCanAction(user.data, task) && (
              <DialogActions className='justify-center'>
                <Button
                  variant='contained'
                  className={`bg-[#EB4242] text-white border-0 hover:bg-red-700 w-24 `}
                  onClick={() => handleOpenSubDialog(task, "approve", false, swrResponse)}
                >
                  Reject
                </Button>
                <Button
                  ...
                  onClick={() => handleOpenSubDialog(task, "approve", true, swrResponse)}
                >
                  Approve
                </Button>
              </DialogActions>
