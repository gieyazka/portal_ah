...
import Action_log from "../../../Components/myAction/action_log";
import Job_Pending from "../../../Components/myAction/job_pending";
...
export default function MyAction(props: { searchParams: { current: string } }) {
  ...
  const currentMenu: menuItem | undefined = menuData.find((d) => d.url === lastPath);
  const filterStore = useFilterStore();
  const searchParams = useSearchParams();
  const [currentSubPath, setCurrentSubPath] = React.useState(searchParams?.get("current") === undefined ? currentMenu !== undefined && currentMenu.subMenu[0].url : searchParams.get("current"));
