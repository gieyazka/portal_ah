...
function LeaveCalendar() {
  ...
  const companyArr = React.useMemo(() => {
    if (query.data) {
      return query.data.map((d) => d.attributes);
    }
    return [];
  }, [query.data]);
  ...
  const departmentArr = React.useMemo(() => {
    if (selectCompany) {
      if (selectCompany.departments.data) {
        return selectCompany.departments.data.map((d) => d.attributes);
      }
    }
    return [];
  }, [selectCompany]);
  ...
  const sectionArr = React.useMemo(() => {
    if (selectDepartment) {
      if (selectDepartment.sections.data) {
        return selectDepartment.sections.data.map((d) => d.attributes);
      }
    }
    return [];
  }, [selectDepartment]);
  const subSectionArr = React.useMemo(() => {
    if (selectSection) {
      if (selectSection.sub_sections.data) {
        return selectSection.sub_sections.data.map((d) => d.attributes);
      }
    }
    return [];
  }, [selectSection]);
...
const MyCalendar = (props: { selectMonth: dayjs.Dayjs; events: any[]; handleClickEvent: () => void }) => {
